...
import { mepStates, mepCities } from "../drizzle/schema";
...
async function seedDatabase() {
  try {
    ...
    for (const state of statesData) {
      try {
        await db.insert(mepStates).values({
    ...
    for (const state of allStates) {
      stateMap.set(state.stateName, state.id);
    }
    ...
    for (const city of citiesData) {
      ...
      try {
        await db.insert(mepCities).values({
          ...
          climateZone: city.climateZone as any,
